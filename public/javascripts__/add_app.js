...
$("#IndivGeo").enterKey(function () {
    var valor=$("#IndivGeo").val();
    var inicio=valor.substring(0, 2);
    var coor;
    if($.isNumeric( inicio )&&valor.length>=7){
    	if(inicio=="-7"||inicio=="10"){
	    	coor=valor.split(";");
	    	//console.log(coor.length);	    	
	    	if(coor.length==1){
	    		Func.msjAlerta("Debe separar la coordenada por punto y coma");
	    	}else{
	    		var cx=coor[0].replace(",",".");
	    		var cy=coor[1].replace(",",".");
				if($.isNumeric( cx )&&$.isNumeric( cy )){    		
		    		if(inicio=="-7"){ var srid=4326; }
		    		else{ var srid=98185; }
		    		var login=Func.DataLogin();
		    		var data= {x:cx,y:cy,srid:srid,id:login.id};
		    		var DataAES =Func.Ecrypted(data);
	    		socket.emit('IndividualInv',{aes:DataAES,idSkt:GeoCode.idSkt});
				}else{
    	...
    	var login=Func.DataLogin();
		var data= {dir:valor,id:login.id};
		var DataAES =Func.Ecrypted(data);
		socket.emit('IndividualDir',{aes:DataAES,idSkt:GeoCode.idSkt});
    }else{
    	Func.msjAlerta("Ingrese un dirección o una coordenada  valida");
    }
...
$("#masivo_inverso").click(function() {
  	$("#MasivoInversoModal").modal("show");
  	$(".navbar-collapse.in").collapse("hide");
  	return false;
});
$("#masivo_directo").click(function() {
	$("#MasivoDirectoModal").modal("show");
  	$(".navbar-collapse.in").collapse("hide");
  	return false;
});
$("#upload-input").hide();
$("#uploadDirecto-input").hide();
Func.setUsrMenu();
Func.IntevaloLogin();
...
$("#CambiarClave").click(function() {
    ...
    if(old_pass==""){
    	Func.msjAlerta("Ingrese la contraseña anterior!");
        $("#old_pass").focus();
        return false;
    }else if(newpass1== ""||newpass2== "") {
        Func.msjAlerta("Ingrese la nueva contraseña!");
        $("#newpass1").focus();
        return false;
    }else if(newpass1!= newpass2) {
        Func.msjAlerta("Las contraseñas no son iguales!");
        $("#newpass1").focus();
        return false;
    }else{
    	var login=Func.DataLogin();
        ...
        socket.on('SetCambioUsuario',function(data){
        	var DataAES =Func.Decrypted(data);
      		//console.log(DataAES);  
      		if(DataAES.cambio=="0"){
      			Func.msjAlerta("Clave anterior errada!!");
...
if(Func.Decrypted(sessionStorage.dt).perfil==1){ //SI ES PERFIL ADMINISTRADOR
	...
	Config.listaUsuarios=function(){
		...
		var $text = $('<div></div>');
			$text.append( '<table class="table"><thead>'+
						      '<tr>'+
						        '<th>Nombre</th>'+
								'<th>Perfil</th>'+
								'<th>Activo</th>'+
	...
	Config.editaUsuario=function(){
		var $text = $('<div></div>');
			$text.append( '<div class="form-group">'+
							...
							'</div>'+
							'<div class="form-group">'+
							  '<label for="fperfil" id="labelPerfil">Perfil</label><select class="form-control" id="fperfil">'+
							  '<option value="">--Seleccione--</option>'+
							  '<option value="1">Administrador</option>'+
							  '<option value="2">Usuario</option>'+
							  '</select>'+
							'</div>'+
							'<div class="form-group">'+
							  '<label for="factivo" id="labelActivo">Activo</label><select class="form-control" id="factivo">'+
							  '<option value="">--Seleccione--</option>'+
        ...
        BootstrapDialog.show({
            ...
            message: $text,
            onshown: function(dialogRef){
            	if(Config['tmp_perfil']==1){
            		$("#factivo,#labelActivo").hide();
            	}else{
            		$("#factivo,#labelActivo").show();
            	}
				$("#factivo").val(Config['tmp_activo']);
				$("#fperfil").val(Config['tmp_perfil']);
				$('#fperfil,#labelPerfil').hide();
